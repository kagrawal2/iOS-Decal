...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    let post = [firImagePathNode: path, firThreadNode: thread, firUsernameNode: username, firDateNode: date]
...
func store(data: Data, toPath path: String) {
    ...
    // YOUR CODE HERE
    storageRef.child(firStorageImagesPath).put(data, metadata: nil, completion: {
        (metadata, error) in
        if let error = error {
            print(error)
        }
        ...
        if(snapshot.exists()) {
            ...
            user.getReadPostIDs(completion: {(readPosts) in
                for (key, object) in dict! {
                    ...
                    let seen = readPosts.contains(key)
                    
                    let postSet = Post(id: key, username: userKey, postImagePath: pathKey, thread: threadKey, dateString: dateKey, read: seen)
                    postArray.append(postSet)
                }
